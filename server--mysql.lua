...
MySQL.ready(function()
    ...
    local mails = MySQL.Sync.fetchAll('SELECT * FROM hsn_phone_mails')
    ...
    Citizen.Wait(10)
    local notes = MySQL.Sync.fetchAll('SELECT * FROM hsn_phone_notes')
    ...
    local accounts = MySQL.Sync.fetchAll('SELECT * FROM hsn_phone_twitter_accounts')
    ...
    local PatientDatas = MySQL.Sync.fetchAll('SELECT * FROM hsn_phone_ambulancepatients')
    ...
    for o,p in pairs(PhoneData) do
        local messages = MySQL.Sync.fetchAll("SELECT owner, number, data, DATE as DATE FROM hsn_phone_messages WHERE `owner` = '"..o.."' ORDER BY DATE DESC") 
        if messages[1] then
...
ESX.RegisterServerCallback("hsn-phone-server-getmessagedata",function(source,cb)
    ...
    local returnData = {}
    local messages = MySQL.Sync.fetchAll("SELECT owner, number, data, DATE as DATE FROM hsn_phone_messages WHERE `owner` = '"..Player.identifier.."' ORDER BY DATE DESC") 
        if messages[1] then
            for k,v in pairs(messages) do
...
HSN.GetPlayerData = function(identifier)
    if PhoneData[identifier] == nil then
        ...
        MySQL.Async.insert('UPDATE users SET phonedata = @phonedata WHERE identifier = @identifier', {
...
HSN.GeneratePhoneNumber = function(identifier) 
    ...
    MySQL.Async.insert('UPDATE users SET phone_number = @phone_number WHERE identifier = @identifier', {
...
HSN.GenerateCryptoId = function(identifier)
    ...
    MySQL.Async.insert('UPDATE users SET cryptoid = @cryptoid WHERE identifier = @identifier', {
        ['@cryptoid'] = generated,
        ['@identifier'] = identifier
    }) 
    return generated
...
AddEventHandler("hsn-phone-server-changePhoto",function(index, photo)
    ...
    if index ~= nil then
        ...
        elseif index == "twitter" then
            ...
            MySQL.Async.insert('UPDATE hsn_phone_twitter_accounts SET photo = @photo WHERE username = @username', {
                ['@photo'] = photo,
                ['@username'] = Username
            })
...
AddEventHandler("hsn-phone-server-deleteApp",function(app)
    ...
    for k,v in pairs(PhoneData[Ply.identifier].Apps) do
        if v == app then
            ...
            MySQL.Async.insert('UPDATE users SET downloadedapps = @downloadedapps WHERE identifier = @identifier', {
...
ESX.RegisterServerCallback("hsn-phone-server-TwitterAccountCheck",function(source,cb,data)
    ...
    if TwitterAccounts[data.username] == nil then
        ...
        if TwitterAccounts[data.username].password == data.password then
            ...
            MySQL.Async.insert('UPDATE hsn_phone_twitter_accounts SET owner = @owner WHERE username = @username', {
...
AddEventHandler("hsn-phone-server-registerTwitter-confirm",function(data)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
	MySQL.Async.insert('INSERT INTO hsn_phone_twitter_accounts (username, password, owner, photo) VALUES (@username, @password, @owner, @photo)', {
		['@username'] = data.username,
...
AddEventHandler("hsn-phone-server-SendNewTweet",function(data)
    if type(data) == "table" then
        ...
        if twtAcc ~= nil then
            ...
            MySQL.Async.insert('INSERT INTO hsn_phone_tweets (username, message, photo, flastname, date) VALUES (@username, @message, @photo, @flastname, @date)', {
...
AddEventHandler("hsn-phone-server-togglelike",function(tweetId)
    ...
    local PlayerData = {}
    local ReturnData = {}
    local result = MySQL.Sync.fetchAll('SELECT * FROM hsn_phone_tweets WHERE id = @id', {
		['@id']  = tweetId
	})
    if TwitterLikes[tweetId] ~= nil then
        ...
        MySQL.Async.insert('UPDATE hsn_phone_tweets SET togglelikes = @togglelikes , likes = @likes WHERE id = @id', {
...
AddEventHandler("hsn-phone-server-twitter-DeleteTwitterAccount",function()
    local UserName = HSN.GetPlayerTwitterUsername(source)
    if UserName ~= nil then
        MySQL.Sync.execute('DELETE FROM hsn_phone_twitter_accounts WHERE username = @username', {
            ['@username'] = UserName,
        })
        if TwitterAccounts[UserName] ~= nil then
...
AddEventHandler("hsn-phone-server-twitter-logOut",function()
    local UserName = HSN.GetPlayerTwitterUsername(source)
    if UserName ~= nil then
        MySQL.Async.insert('UPDATE `hsn_phone_twitter_accounts` SET `owner`= "" WHERE username = @username', {
            ['@username'] = UserName
        }, function()
...
ESX.RegisterServerCallback("hsn-phone-server-getTweets",function(source,cb)
    local returnData = {}
    local tweets = MySQL.Sync.fetchAll('SELECT * FROM hsn_phone_tweets', {})
...
AddEventHandler("hsn-phone-server-deletenote",function(noteid)
    ...
    for k,v in pairs(PhoneData[Player.identifier].notes) do
        if v.id == tonumber(noteid) then
            ...
            MySQL.Async.insert('DELETE FROM hsn_phone_notes WHERE id = @id', {
                ['@id'] = tonumber(noteid)
            })
            break
        end
...
HSN.AddNewNote = function(src,noteData)
    local Player = ESX.GetPlayerFromId(src)
    MySQL.Async.insert('INSERT INTO hsn_phone_notes (owner, noteData) VALUES (@owner, @noteData)', {
        ['@owner']   = Player.identifier,
...
AddEventHandler("hsn-phone-server-addnewContact",function(data)
    ...
    if data.phonenumber and data.name then
        ...
        if TargetPlayer ~= "" then
            ...
            MySQL.Async.insert('UPDATE users SET contacts = @contacts WHERE identifier = @identifier', {
...
AddEventHandler("hsn-phone-server-addNewMail",function(data)
    ...
    MySQL.Async.insert('INSERT INTO hsn_phone_mails (owner, message,date,app) VALUES (@owner, @message, @date, @app)', {
        ['@owner']   = Player.identifier,
        ['@message']   = data.message,
        ["@date"] = data.date,
        ["@app"] = data.app
...
RegisterCommand("testcall",function(source)
    local callid = HSN.GenerateCallId()
    PhoneCalls[callid] = {}
    
    TriggerClientEvent("hsn-phone-client:NewNotification",source,{type = "call-src", message = "Hasan Test aranıyor!", targetNumber = 11111})
end)
...
AddEventHandler("hsn-phone-server-deletemail",function(id)
    ...
    for k,v in pairs(PhoneData[Player.identifier].mails) do
        if v.id == tonumber(id) then
            ...
            MySQL.Sync.execute('DELETE FROM hsn_phone_mails WHERE id = @id', {
                ['@id'] = tonumber(id)
            })
            ...
            if ESX.GetPlayerFromIdentifier(Player.identifier) then
               if ESX.GetPlayerFromIdentifier(Player.identifier).source == source then
                ...
                if NumberInContact(srcPlayer.identifier, number) then
                    TriggerClientEvent("hsn-phone-client:NewNotification",source,{type = "call-src", message = fullName.." aranıyor!", targetNumber = Player.phonenumber, id = callid})
                else
                    TriggerClientEvent("hsn-phone-client:NewNotification",source,{type = "call-src", message = number.." aranıyor!", targetNumber = Player.phonenumber, id = callid})
                end
                if anonymouscall then
                    TriggerClientEvent("hsn-phone-client:NewNotification",ESX.GetPlayerFromIdentifier(Player.identifier).source,{type = "call-target", message = "Unknown Number arıyor!", targetNumber = Player.phonenumber, id = callid})
                else
                    if NumberInContact(Player.identifier, PhoneData[srcPlayer.identifier].phonenumber) then
                        TriggerClientEvent("hsn-phone-client:NewNotification",ESX.GetPlayerFromIdentifier(Player.identifier).source,{type = "call-target", message = fullName.." arıyor!", targetNumber = Player.phonenumber, id = callid})
                    else
                        TriggerClientEvent("hsn-phone-client:NewNotification",ESX.GetPlayerFromIdentifier(Player.identifier).source,{type = "call-target", message = PhoneData[srcPlayer.identifier].phonenumber.." arıyor!", targetNumber = Player.phonenumber, id = callid})
                    end
                end
            else
                TriggerClientEvent("hsn-phone-client-call", source, false, "The person is not available!")
            ...
            if cryptoData[crypto] then
                if type == "buy" then
                    if Player.getAccount("bank").money >= amount then
                        ...
                        MySQL.Async.insert('UPDATE users SET cryptocurrency = @cryptocurrency WHERE identifier = @identifier', {
        ...
        if cryptoData then
            if cryptoData[crypto] then
                ...
                if PhoneData[Player.identifier].CryptoCurrency[crypto] >= amount then
                    ...
                    MySQL.Async.insert('UPDATE users SET cryptocurrencytransfers = @cryptocurrencytransfers WHERE identifier = @identifier', {
...
AddEventHandler("hsn-phone-server-CreateNewPatientData",function(patientData)
    ...
    if patientData then
        if patientData.name ~= "" and patientData.message ~= "" then
            ...
            MySQL.Async.insert('INSERT INTO hsn_phone_ambulancepatients (name, message,photo,date, id) VALUES (@name, @message, @photo, @date, @id)', {
    ...
    if patientData and patientData.patient then
        ...
        for k,v in pairs(AmbulancePatientDatas) do
            if v.id == patientData.patient then
                ...
                print("asdasdasd")
                MySQL.Async.insert('DELETE FROM hsn_phone_ambulancepatients WHERE id = @id', {
...
ESX.RegisterServerCallback("hsn-phone-server-SearchCitizen",function(source, cb, searchData)
    local playersTable = {}
    local result = MySQL.Sync.fetchAll("SELECT * FROM users WHERE CONCAT(firstname, ' ', lastname) LIKE '%"..searchData.."%' OR phone_number LIKE '%"..searchData.."%'")
    if result[1] ~= nil then
        for k,v in pairs(result) do
            if v.phonedata ~= nil then
...
ESX.RegisterServerCallback("hsn-phone-server-SearchVehicle",function(source, cb, searchData)
    local vehicleTable = {}
    local result = MySQL.Sync.fetchAll("SELECT * FROM owned_vehicles WHERE CONCAT(plate) LIKE '%"..searchData.."%'")
    if result[1] ~= nil then
        for k,v in pairs(result) do
    ...
    if bolo then
        if bolo.reason then
            if bolo.player ~= nil or bolo.player ~= "0" or bolo.player ~= 0 then
                ...
                MySQL.Async.insert('UPDATE users SET bolos = @bolos WHERE identifier = @identifier', {
...
AddEventHandler("hsn-phone-server-SendMessage",function(data)
    ...
    local result = MySQL.Sync.fetchAll('SELECT * FROM hsn_phone_messages WHERE owner = @owner AND number = @number', {
		['@owner']  = Player.identifier,
        ['@number'] = data.number
	})
    if result[1] then
        MySQL.Async.insert('UPDATE hsn_phone_messages SET data = @data, date = @date, number = @number WHERE owner = @owner AND number = @number', {
        ...
        MySQL.Async.insert('INSERT INTO hsn_phone_messages (data, owner, number, date) VALUES (@data, @owner, @number, @date)', {
            ['@owner']   = Player.identifier,
            ['@data']   = json.encode(PhoneData[Player.identifier].messages[data.number]),
            ['@number'] = data.number,
            ['@date'] = os.date('%Y-%m-%d %H:%M:%S')
